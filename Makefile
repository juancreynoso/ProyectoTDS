# Compilador
CC = gcc
CFLAGS = -Wall -g -I. -Iast -Ist -Isintax -Ilexer -Isemantic -Itac -Iass -Icompile

# Archivos y carpetas
BISON = bison -v -d
FLEX = flex

SRC_DIRS = sintax lexer ast semantic compile tac opt
OBJS = sintax/sintax.tab.o lexer/lex.yy.o ast/ast.o ast/set_offsets.o main.o semantic/semantic_analyzer.o semantic/type_check.o semantic/symbol_table.o common.o ast/param_list.o tac/tac_generator.o ass/assembler_generator.o opt/constants_propagation.o
TARGET = c-tds

# Carpeta de archivos de salida
OUTDIR = outputs

# Regla principal
all: $(OUTDIR) $(TARGET) 

# Crear 
$(OUTDIR): 
	mkdir -p $(OUTDIR)

# Ejecutable final
$(TARGET): $(OBJS)
	$(CC) $(CFLAGS) -o $@ $(OBJS)

# Documentacion
open-doc: doc/index.html
	open doc/index.html

# Generar parser con bison
sintax/sintax.tab.c sintax/sintax.tab.h: sintax/sintax.y
	$(BISON) -o sintax/sintax.tab.c $<

# Generar lexer con Flex
lexer/lex.yy.c: lexer/lexer.l
	$(FLEX) -o $@ $<

# Compilar cada objeto
sintax/sintax.tab.o: sintax/sintax.tab.c
	${CC} ${CFLAGS} -c -o $@ $<

lexer/lex.yy.o: lexer/lex.yy.c
	${CC} ${CFLAGS} -c -o $@ $<

ast/param_list.o: ast/param_list.c ast/param_list.h
	${CC} ${CFLAGS} -c -o $@ $<

ast/ast.o: ast/ast.c ast/ast.h
	${CC} ${CFLAGS} -c -o $@ $<

ast/set_offsets.o : ast/set_offsets.c ast/set_offsets.h
	${CC} ${CFLAGS} -c -o $@ $<

semantic/semantic_analyzer.o: semantic/semantic_analyzer.c semantic/semantic_analyzer.h
	${CC} ${CFLAGS} -c -o $@ $<

semantic/symbol_table.o: semantic/symbol_table.c semantic/symbol_table.h
	${CC} ${CFLAGS} -c -o $@ $<

semantic/type_check.o: semantic/type_check.c semantic/type_check.h
	${CC} ${CFLAGS} -c -o $@ $<

common.o: common.c
	${CC} ${CFLAGS} -c -o $@ $<
	
tac/tac_generator.o: tac/tac_generator.c tac/tac_generator.h
	${CC} ${CFLAGS} -c -o $@ $<

ass/assembler_generator.o: ass/assembler_generator.c ass/assembler_generator.h
	${CC} ${CFLAGS} -c -o $@ $<

opt/constants_propagation.o: opt/constants_propagation.c opt/constants_propagation.h
	${CC} ${CFLAGS} -c -o $@ $<
	
main.o: main.c
	${CC} ${CFLAGS} -c -o $@ $<

# Limpiar archivos generados
clean:
	rm -f $(OBJS) $(TARGET) sintax/sintax.tab.* lexer/lex.yy.c sintax/sintax.output
	rm -rf $(OUTDIR)