            ...
            switch (root->info->OP.name) {
                case OP_NOT:
                    if (leftType != TYPE_BOOL) {
                            printf("Error de tipos. Operacion %s  requiere tipo booleano\n", op_name(root->info->OP.name));
                            exit(EXIT_FAILURE);
                    }
                ...
                case OP_MINUS:
                    if (leftType != TYPE_INT) {
                            printf("Error de tipos. Operacion %s  requiere tipo entero\n", op_name(root->info->OP.name));
                            exit(EXIT_FAILURE);
                    }
                    return root->info->OP.type = leftType;
                    break;
        ...
        switch (root->info->OP.name) {
            ...
            case OP_ASSIGN:
                if(leftType != rightType) {
                    printf("Error: tipos incompatibles en asignacion, Esperado '%s', Recibido: '%s' \n", 
